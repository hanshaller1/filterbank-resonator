...
export class ModulationPanelV2 {
  constructor(store, root) {
    ...
    root.innerHTML = `<div class="panel-title"><h2>Modulation</h2><span>Grundwert + Modulation · begrenzte Zielbereiche</span><button class="module-default" type="button" data-reset-module="modulation">Default</button></div><div class="modulation-modules">${lfoMarkup('lfo1', 'LFO 1')}${lfoMarkup('lfo2', 'LFO 2')}${envelopeMarkup('envelope1', 'Envelope Follower 1')}${envelopeMarkup('envelope2', 'Envelope Follower 2')}</div>
  ...
  resetUi() { this.visual = null; this.lastTempo = null; for (const history of Object.values(this.history)) { history.input.length = 0; history.envelope.length = 0; } }
